...
resilience_finality_timeout="${MYOSU_E2E_RESILIENCE_FINALITY_TIMEOUT:-120}"
...
start_authority() {
  ...
  local cmd=(
    ...
    --validator
    --force-authoring
    --name "myosu-four-node-authority-${idx}"
...
mkdir -p "$work_parent"
work_root="$(mktemp -d "$work_parent/four-node-finality.XXXXXX")"
...
pre_kill_finalized_block="${shared_finalized_blocks[1]}"

printf 'FINALITY before_kill hash=%s\n' "$shared_finalized_hash"
for idx in 1 2 3 4; do
  printf 'FINALITY node=%s best=%s finalized=%s peers=%s rpc=%s\n' \
    "$idx" \
...
post_kill_target=$((pre_kill_finalized_block + 1))
wait_for_shared_finality "$resilience_finality_timeout" "$post_kill_target" 1 2 3

printf 'FINALITY after_kill hash=%s\n' "$shared_finalized_hash"
for idx in 1 2 3; do
  printf 'FINALITY node=%s best=%s finalized=%s peers=%s rpc=%s\n' \
    "$idx" \
    "${shared_best_blocks[$idx]}" \
...
printf 'FINALITY four_node_devnet ok\n'
