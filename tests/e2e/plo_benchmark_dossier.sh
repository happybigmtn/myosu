#!/usr/bin/env bash
# F-017 PLO benchmark dossier proof.
#
# The dossier is the promotion evidence for the F-017 slice (the
# F-001/F-008/F-009/F-010/F-011/F-012/F-013/F-014/F-015/F-016-pattern
# eleventh portfolio-game-promotion slice after Cribbage / Hearts /
# Gin Rummy / Spades / Bridge / Call Break / Backgammon / Hanafuda
# Koi-Koi / Hwatu Go-Stop / Stratego): it pins the 22-scenario
# rule-aware scenario pack for PLO, runs the live portfolio engine
...
# requirements (R1, R2, R3, R4) hold for PLO in the same way they
# hold for the ten prior portfolio slices. F-017 also opens the
# `state-aware PLO nut-draw heuristic` engine sub-family in
# `crates/myosu-games-portfolio/src/engines/poker_like.rs::plo` —
# the first dossier slice for the `poker_like` engine family, and
# the natural one because PLO is the closest cousin to the dedicated
# `nlhe-heads-up` engine.
#
...
#      is one of draw-to-nuts / pot-sized-raise / pot-control.
#   3. The promotion ledger (`ops/solver_promotion.yaml`) declares
#      PLO at `tier: benchmarked` and `bundle_support: benchmarked`.
#   4. The promotion manifest harness passes with PLO visible at
#      `tier=benchmarked code_bundle_support=benchmarked`.
#   5. The PLO benchmark unit tests in `myosu-games-portfolio`
#      (dossier build, determinism, recommendation completeness,
#      draw-to-nuts dominance, pot-sized-raise dominance,
#      pot-control dominance, scenario count) all pass.
...
mkdir -p "$work_parent"
work_root="$(mktemp -d "$work_parent/plo-benchmark-dossier.XXXXXX")"
trap 'rm -rf "$work_root"' EXIT

dossier_output_dir="$work_root/outputs/solver-promotion/plo"
...
example_output="$(
    env \
        SKIP_WASM_BUILD=1 \
        MYOSU_PLO_BENCHMARK_OUTPUT="$dossier_output_dir" \
        cargo run --quiet -p myosu-games-portfolio --example plo_benchmark
)"

if ! printf '%s\n' "$example_output" | grep -Fq 'PLO_BENCHMARK status=ok'; then
    printf 'plo_benchmark example did not report status=ok\n' >&2
    printf '%s\n' "$example_output" >&2
    exit 1
fi

if ! printf '%s\n' "$example_output" | grep -Eq 'scenario_count=22[[:space:]]+recommendation_count=22.*passing=yes'; then
    printf 'plo_benchmark did not report 22/22 passing=yes\n' >&2
    printf '%s\n' "$example_output" >&2
    exit 1
fi

# -- 2. The on-disk dossier is well-formed and has the expected hash.
dossier_path="$dossier_output_dir/plo-benchmark-dossier.json"
if [[ ! -s "$dossier_path" ]]; then
    printf 'plo_benchmark did not write dossier JSON at %s\n' "$dossier_path" >&2
    exit 1
fi
...
if len(d["scenario_hash"]) != 64:
    ...
    sys.exit(2)
allowed_actions = {"draw-to-nuts", "pot-sized-raise", "pot-control"}
...
if [[ -z "$dossier_summary" ]] || ! printf '%s\n' "$dossier_summary" | grep -Fq 'DOSSIER_OK'; then
    printf 'plo dossier failed shape check\n' >&2
    exit 1
fi
printf '%s\n' "$dossier_summary"
...
if [[ -z "$hash_first" || -z "$hash_summary" || "$hash_first" != "$hash_summary" ]]; then
    printf 'plo scenario_hash drift: stdout=%s summary=%s\n' \
        "$hash_first" "$hash_summary" >&2
    exit 1
fi

# -- 3. The promotion ledger declares PLO at tier=benchmarked.
ledger="$repo_root/ops/solver_promotion.yaml"
ledger_row="$(
    awk '
        /^  - game: plo$/ { found=1; next }
...
if ! printf '%s\n' "$ledger_row" | grep -Fxq 'TIER=benchmarked'; then
    printf 'plo ledger tier is not benchmarked\n%s\n' "$ledger_row" >&2
    exit 1
fi
if ! printf '%s\n' "$ledger_row" | grep -Fxq 'BUNDLE=benchmarked'; then
    printf 'plo ledger bundle_support is not benchmarked\n%s\n' "$ledger_row" >&2
    exit 1
fi

# -- 4. The promotion manifest harness passes with PLO at benchmarked.
manifest_output="$(
    env \
        SKIP_WASM_BUILD=1 \
...
plo_manifest_line="$(
    printf '%s\n' "$manifest_output" \
        | grep -E '^SOLVER_PROMOTION_GAME slug=plo ' \
        || true
)"
if [[ -z "$plo_manifest_line" ]]; then
    printf 'promotion manifest is missing the plo row\n%s\n' "$manifest_output" >&2
    exit 1
fi
for needle in 'tier=benchmarked' 'code_bundle_support=benchmarked' 'benchmark_surface=rule_aware_scenario_pack'; do
    if ! printf '%s\n' "$plo_manifest_line" | grep -Fq "$needle"; then
        printf 'plo manifest row missing %s\n%s\n' "$needle" "$plo_manifest_line" >&2
        exit 1
    fi
done

# -- 5. The portfolio crate's PLO unit tests all pass.
unit_output="$(
    env SKIP_WASM_BUILD=1 cargo test --quiet -p myosu-games-portfolio -- plo_dossier plo_scenario_pack
)"
if ! printf '%s\n' "$unit_output" | grep -Fq 'test result: ok.'; then
    printf 'plo_dossier / plo_scenario_pack unit tests did not all pass\n%s\n' "$unit_output" >&2
    exit 1
fi
unit_pass_line="$(
    printf '%s\n' "$unit_output" \
...
printf 'PLO_BENCHMARK_HARNESS plo unit tests: %s\n' "$unit_pass_line"

printf 'PLO_BENCHMARK_HARNESS myosu e2e ok scenario_hash=%s ledger=%s\n' \
    "$hash_first" "$ledger"
