#!/usr/bin/env bash
# F-019 NLHE-six-max benchmark dossier proof.
#
# The dossier is the promotion evidence for the F-019 slice (the
# F-001/F-008/F-009/F-010/F-011/F-012/F-013/F-014/F-015/F-016/F-017/F-018-pattern
# thirteenth portfolio-game-promotion slice after Cribbage / Hearts /
# Gin Rummy / Spades / Bridge / Call Break / Backgammon / Hanafuda
# Koi-Koi / Hwatu Go-Stop / Stratego / PLO / Dou-Di-Zhu): it pins the
# 22-scenario rule-aware scenario pack for NLHE-six-max, runs the live
...
# `tier: benchmarked` claim matches the live engine output. The ledger
# (R3) and manifest harness (R4) are the supporting surfaces; this
# harness is the executable end-to-end proof that all four
# requirements (R1, R2, R3, R4) hold for NLHE-six-max in the same
# way they hold for the twelve prior portfolio slices. F-019 opens a
# second dossier row for the `state-aware poker range heuristic`
# engine surface in `crates/myosu-games-portfolio/src/engines/poker_like.rs::nlhe_six_max`
# (the F-017 PLO row was the first dossier slice for the
# `poker_like` engine family, but its three arms — `draw_to_nuts` /
# `pot_sized_raise` / `pot_control` — are disjoint from the F-019
# NLHE-six-max arms — `value_bet` / `tight_open` / `pot_control`).
#
...
#      is one of value-bet / tight-open / pot-control.
#   3. The promotion ledger (`ops/solver_promotion.yaml`) declares
#      NLHE-six-max at `tier: benchmarked` and `bundle_support:
#      benchmarked`.
#   4. The promotion manifest harness passes with NLHE-six-max
#      visible at `tier=benchmarked code_bundle_support=benchmarked`.
#   5. The NLHE-six-max benchmark unit tests in `myosu-games-portfolio`
#      (dossier build, determinism, recommendation completeness,
#      value-bet dominance, tight-open dominance, pot-control
...
mkdir -p "$work_parent"
work_root="$(mktemp -d "$work_parent/nlhe-six-max-benchmark-dossier.XXXXXX")"
trap 'rm -rf "$work_root"' EXIT

dossier_output_dir="$work_root/outputs/solver-promotion/nlhe-six-max"
...
example_output="$(
    env \
        SKIP_WASM_BUILD=1 \
        MYOSU_NLHE_SIX_MAX_BENCHMARK_OUTPUT="$dossier_output_dir" \
        cargo run --quiet -p myosu-games-portfolio --example nlhe_six_max_benchmark
)"

if ! printf '%s\n' "$example_output" | grep -Fq 'NLHE_SIX_MAX_BENCHMARK status=ok'; then
    printf 'nlhe_six_max_benchmark example did not report status=ok\n' >&2
    printf '%s\n' "$example_output" >&2
    exit 1
fi

if ! printf '%s\n' "$example_output" | grep -Eq 'scenario_count=22[[:space:]]+recommendation_count=22.*passing=yes'; then
    printf 'nlhe_six_max_benchmark did not report 22/22 passing=yes\n' >&2
    printf '%s\n' "$example_output" >&2
    exit 1
fi

# -- 2. The on-disk dossier is well-formed and has the expected hash.
dossier_path="$dossier_output_dir/nlhe-six-max-benchmark-dossier.json"
if [[ ! -s "$dossier_path" ]]; then
    printf 'nlhe_six_max_benchmark did not write dossier JSON at %s\n' "$dossier_path" >&2
    exit 1
fi
...
if len(d["scenario_hash"]) != 64:
    ...
    sys.exit(2)
allowed_actions = {"value-bet", "tight-open", "pot-control"}
...
if [[ -z "$dossier_summary" ]] || ! printf '%s\n' "$dossier_summary" | grep -Fq 'DOSSIER_OK'; then
    printf 'nlhe_six_max dossier failed shape check\n' >&2
    exit 1
fi
printf '%s\n' "$dossier_summary"
...
if [[ -z "$hash_first" || -z "$hash_summary" || "$hash_first" != "$hash_summary" ]]; then
    printf 'nlhe_six_max scenario_hash drift: stdout=%s summary=%s\n' \
        "$hash_first" "$hash_summary" >&2
    exit 1
fi

# -- 3. The promotion ledger declares NLHE-six-max at tier=benchmarked.
ledger="$repo_root/ops/solver_promotion.yaml"
ledger_row="$(
    awk '
        /^  - game: nlhe-six-max$/ { found=1; next }
...
if ! printf '%s\n' "$ledger_row" | grep -Fxq 'TIER=benchmarked'; then
    printf 'nlhe-six-max ledger tier is not benchmarked\n%s\n' "$ledger_row" >&2
    exit 1
fi
if ! printf '%s\n' "$ledger_row" | grep -Fxq 'BUNDLE=benchmarked'; then
    printf 'nlhe-six-max ledger bundle_support is not benchmarked\n%s\n' "$ledger_row" >&2
    exit 1
fi

# -- 4. The promotion manifest harness passes with NLHE-six-max at benchmarked.
manifest_output="$(
    env \
        SKIP_WASM_BUILD=1 \
...
nlhe_six_max_manifest_line="$(
    printf '%s\n' "$manifest_output" \
        | grep -E '^SOLVER_PROMOTION_GAME slug=nlhe-six-max ' \
        || true
)"
if [[ -z "$nlhe_six_max_manifest_line" ]]; then
    printf 'promotion manifest is missing the nlhe-six-max row\n%s\n' "$manifest_output" >&2
    exit 1
fi
for needle in 'tier=benchmarked' 'code_bundle_support=benchmarked' 'benchmark_surface=rule_aware_scenario_pack'; do
    if ! printf '%s\n' "$nlhe_six_max_manifest_line" | grep -Fq "$needle"; then
        printf 'nlhe-six-max manifest row missing %s\n%s\n' "$needle" "$nlhe_six_max_manifest_line" >&2
        exit 1
    fi
done

# -- 5. The portfolio crate's NLHE-six-max unit tests all pass.
unit_output="$(
    env SKIP_WASM_BUILD=1 cargo test --quiet -p myosu-games-portfolio -- nlhe_six_max_dossier nlhe_six_max_scenario_pack
)"
if ! printf '%s\n' "$unit_output" | grep -Fq 'test result: ok.'; then
    printf 'nlhe_six_max_dossier / nlhe_six_max_scenario_pack unit tests did not all pass\n%s\n' "$unit_output" >&2
    exit 1
fi
unit_pass_line="$(
    printf '%s\n' "$unit_output" \
...
printf 'NLHE_SIX_MAX_BENCHMARK_HARNESS nlhe-six-max unit tests: %s\n' "$unit_pass_line"

printf 'NLHE_SIX_MAX_BENCHMARK_HARNESS myosu e2e ok scenario_hash=%s ledger=%s\n' \
    "$hash_first" "$ledger"
