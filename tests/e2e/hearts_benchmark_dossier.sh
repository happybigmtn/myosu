#!/usr/bin/env bash
# F-008 Hearts benchmark dossier proof.
#
# The dossier is the promotion evidence for the F-008 slice (the F-001-pattern
# next-portfolio-game-promotion slice after Cribbage): it pins the 22-scenario
# rule-aware scenario pack, runs the live portfolio engine against it, records
# every engine recommendation, and SHA-256-pins the canonical scenario/answer
# table so the promotion manifest harness can verify that the evidence
# attached to a `tier: benchmarked` claim matches the live engine output. The
# ledger (R3) and manifest harness (R4) are the supporting surfaces; this
# harness is the executable end-to-end proof that all four requirements
# (R1, R2, R3, R4) hold for Hearts in the same way they hold for Cribbage.
#
...
#      action token is one of avoid-penalty / follow-suit / shoot-moon.
#   3. The promotion ledger (`ops/solver_promotion.yaml`) declares
#      Hearts at `tier: benchmarked` and `bundle_support: benchmarked`.
#   4. The promotion manifest harness passes with 22 rows and Hearts
#      visible at `tier=benchmarked code_bundle_support=benchmarked`.
#   5. The Hearts benchmark unit tests in `myosu-games-portfolio`
#      (dossier build, determinism, recommendation completeness,
#      moon-viable shoot-moon dominance, forced-follow follow-suit
#      dominance) all pass.
...
mkdir -p "$work_parent"
work_root="$(mktemp -d "$work_parent/hearts-benchmark-dossier.XXXXXX")"
trap 'rm -rf "$work_root"' EXIT

dossier_output_dir="$work_root/outputs/solver-promotion/hearts"
...
example_output="$(
    env \
        SKIP_WASM_BUILD=1 \
        MYOSU_HEARTS_BENCHMARK_OUTPUT="$dossier_output_dir" \
        cargo run --quiet -p myosu-games-portfolio --example hearts_benchmark
)"

if ! printf '%s\n' "$example_output" | grep -Fq 'HEARTS_BENCHMARK status=ok'; then
    printf 'hearts_benchmark example did not report status=ok\n' >&2
    printf '%s\n' "$example_output" >&2
    exit 1
fi

if ! printf '%s\n' "$example_output" | grep -Eq 'scenario_count=22[[:space:]]+recommendation_count=22.*passing=yes'; then
    printf 'hearts_benchmark did not report 22/22 passing=yes\n' >&2
    printf '%s\n' "$example_output" >&2
    exit 1
fi

# -- 2. The on-disk dossier is well-formed and has the expected hash.
dossier_path="$dossier_output_dir/hearts-benchmark-dossier.json"
if [[ ! -s "$dossier_path" ]]; then
    printf 'hearts_benchmark did not write dossier JSON at %s\n' "$dossier_path" >&2
    exit 1
fi
...
if len(d["scenario_hash"]) != 64:
    ...
    sys.exit(2)
allowed_actions = {"avoid-penalty", "follow-suit", "shoot-moon"}
...
if [[ -z "$dossier_summary" ]] || ! printf '%s\n' "$dossier_summary" | grep -Fq 'DOSSIER_OK'; then
    printf 'hearts dossier failed shape check\n' >&2
    exit 1
fi
printf '%s\n' "$dossier_summary"
...
if [[ -z "$hash_first" || -z "$hash_summary" || "$hash_first" != "$hash_summary" ]]; then
    printf 'hearts scenario_hash drift: stdout=%s summary=%s\n' \
        "$hash_first" "$hash_summary" >&2
    exit 1
fi

# -- 3. The promotion ledger declares Hearts at tier=benchmarked.
ledger="$repo_root/ops/solver_promotion.yaml"
ledger_row="$(
    awk '
        /^  - game: hearts$/ { found=1; next }
...
if ! printf '%s\n' "$ledger_row" | grep -Fxq 'TIER=benchmarked'; then
    printf 'hearts ledger tier is not benchmarked\n%s\n' "$ledger_row" >&2
    exit 1
fi
if ! printf '%s\n' "$ledger_row" | grep -Fxq 'BUNDLE=benchmarked'; then
    printf 'hearts ledger bundle_support is not benchmarked\n%s\n' "$ledger_row" >&2
    exit 1
fi

# -- 4. The promotion manifest harness passes with Hearts at benchmarked.
manifest_output="$(
    env \
        SKIP_WASM_BUILD=1 \
...
hearts_manifest_line="$(
    printf '%s\n' "$manifest_output" \
        | grep -E '^SOLVER_PROMOTION_GAME slug=hearts ' \
        || true
)"
if [[ -z "$hearts_manifest_line" ]]; then
    printf 'promotion manifest is missing the hearts row\n%s\n' "$manifest_output" >&2
    exit 1
fi
for needle in 'tier=benchmarked' 'code_bundle_support=benchmarked' 'benchmark_surface=rule_aware_scenario_pack'; do
    if ! printf '%s\n' "$hearts_manifest_line" | grep -Fq "$needle"; then
        printf 'hearts manifest row missing %s\n%s\n' "$needle" "$hearts_manifest_line" >&2
        exit 1
    fi
done

# -- 5. The portfolio crate's Hearts unit tests all pass.
unit_output="$(
    env SKIP_WASM_BUILD=1 cargo test --quiet -p myosu-games-portfolio -- hearts
)"
if ! printf '%s\n' "$unit_output" | grep -Fq 'test result: ok.'; then
    printf 'hearts unit tests did not all pass\n%s\n' "$unit_output" >&2
    exit 1
fi
unit_pass_line="$(
    printf '%s\n' "$unit_output" \
...
printf 'HEARTS_BENCHMARK_HARNESS hearts unit tests: %s\n' "$unit_pass_line"

# Also run the targeted HeartsBenchmarkDossier unit tests with a substring
# filter so the harness names them explicitly.
dossier_unit="$(
    env SKIP_WASM_BUILD=1 cargo test --quiet -p myosu-games-portfolio -- hearts_dossier
)"
if ! printf '%s\n' "$dossier_unit" | grep -Fq 'test result: ok.'; then
    printf 'hearts_dossier unit tests did not all pass\n%s\n' "$dossier_unit" >&2
    exit 1
fi

printf 'HEARTS_BENCHMARK_HARNESS myosu e2e ok scenario_hash=%s ledger=%s\n' \
    "$hash_first" "$ledger"
