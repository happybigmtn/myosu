...
- `.github/workflows/ci.yml` `integration-e2e` now runs `local_loop.sh`, `validator_determinism.sh`, and `emission_flow.sh`; warm local timings are about 3m08 + 2m32 + 3m08 before checkout/toolchain/protoc overhead, so keep that job timeout at 15 minutes unless one of those proofs is removed or sped up
