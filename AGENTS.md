...
- `.github/workflows/ci.yml` `integration-e2e` now runs `local_loop.sh`, `two_node_sync.sh`, `four_node_finality.sh`, `cross_node_emission.sh`, `validator_determinism.sh`, and `emission_flow.sh`; keep the sync/finality/emission proofs after `local_loop.sh` so they can reuse the devnet-built `myosu-chain` / runtime artifacts, and budget several minutes beyond the old five-proof ~10m44 warm local baseline when evaluating the 15-minute timeout
