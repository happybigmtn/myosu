...
- Fresh local devnet start-up is not instant: JSON-RPC on `127.0.0.1:9955` usually appears only after genesis + service initialization (~15s on this repo), so `tests/e2e/helpers/wait_for_block.sh` should keep its full startup timeout instead of assuming immediate RPC readiness
