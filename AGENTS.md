...
- Root-level `PY-*` proof commands currently assume the default `python` interpreter already has `numpy` and `ruff`; the repo still does not manage Python tool dependencies, so install them once with `python -m pip install numpy ruff` before running the Python quality-gate checks
