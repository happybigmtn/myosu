...
- `P-011` is currently blocked by a real 3-node finality stall, not just a missing script: on 2026-04-05, ad hoc full-mesh repros with unique Prometheus ports showed both the default `litep2p` backend and explicit `--network-backend libp2p` freezing finalized height at `#2` after killing one of three authorities while best blocks kept importing past `#10`
- Ad hoc 3-authority `devnet` repros need a distinct generated `--node-key-file` for every authority base path; otherwise non-boot authorities can exit before RPC comes up with `NetworkKeyNotFound(.../network/secret_ed25519)` instead of exercising the consensus bug
- The backend-agnostic P-011 repro now also has a stronger runtime symptom: with proper per-authority node keys, both `litep2p` and explicit `--network-backend libp2p` still freeze finalized height at `#2` after killing `authority-3`, and the `libp2p` run logs a GRANDPA prevote equivocation for `authority-2`
...
- `.github/workflows/ci.yml` `integration-e2e` now runs `local_loop.sh`, `two_node_sync.sh`, `validator_determinism.sh`, and `emission_flow.sh`; keep `two_node_sync.sh` after `local_loop.sh` so it can reuse the devnet-built `myosu-chain` / runtime artifacts, and budget about 3m22 + 0m36 + 2m28 + 2m56 warm local time (~9m21 total) before checkout/toolchain/protoc overhead when evaluating the 15-minute timeout
