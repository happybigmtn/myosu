...
- `tests/e2e/local_loop.sh` and `tests/e2e/validator_determinism.sh` are determinism/wiring proofs, not solver-quality benchmarks: both feed the miner-produced checkpoint back into `myosu-validator --checkpoint`, so the expected stage-0 happy path is `exact_match=true` / `score=1.0` when the response came from that checkpoint. Use exploitability or an independent reference checkpoint for any minimum-iterations / convergence work instead of those scripts.
- The repo-owned poker bootstrap artifact path is intentionally sparse: `crates/myosu-games-poker/examples/bootstrap_artifacts.rs` emits the same single-lookup encoder that `crates/myosu-miner/src/training.rs` covers with `run_training_batch_reports_sparse_encoder_failure_cleanly`, so any poker `--train-iterations > 0` run on those bootstrap artifacts fails upstream with `isomorphism not found`
- Generated `target/e2e/*/poker/encoder/manifest.json` surfaces are just copies of that bootstrap artifact shape (`preflop.entries = 1`, no flop/turn/river files); do not treat them as richer poker encoders when evaluating `F-007` or any positive-iteration poker benchmark
...
- `bash tests/e2e/validator_determinism.sh` now covers both supported validator games (`poker` and `liars-dice`) in one run; use `MYOSU_E2E_GAMES='liars-dice'` or `MYOSU_E2E_GAMES='poker'` to narrow the proof while iterating locally
