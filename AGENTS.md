...
- `.github/workflows/ci.yml` `integration-e2e` now runs `local_loop.sh`, `two_node_sync.sh`, `four_node_finality.sh`, `validator_determinism.sh`, and `emission_flow.sh`; keep the sync/finality proofs after `local_loop.sh` so they can reuse the devnet-built `myosu-chain` / runtime artifacts, and budget about 3m22 + 0m36 + 1m22 + 2m28 + 2m56 warm local time (~10m44 total) before checkout/toolchain/protoc overhead when evaluating the 15-minute timeout
