...
cargo build -p myosu-node --release` exits 0.`
  ...
  - Stderr:
    ...
    /bin/bash: line 2: on: command not found
     Downloading crates ...
      Downloaded finality-grandpa v0.16.3
    error: package ID specification `myosu-node` did not match any packages
    error: unexpected argument '--releasecargo' found
...
- failure_signature: verify|deterministic|script failed with exit code: <n> ## stderr /bin/bash: line <n>: exits: command not found /bin/bash: line <n>: on: command not found downloading crates ... downloaded finality-grandpa v0.<n>.<n> error: package id specification `myosu-node` 
