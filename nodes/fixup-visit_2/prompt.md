...
cargo test -p myosu-play advisor::tests::format_distribution_text`
  ...
  - Stderr:
    ...
    ```
- **fixup**: success
  - Model: MiniMax-M2.7-highspeed, 105.3k tokens in / 4.3k out
  - Files: outputs/play/tui/verification.md
- **verify**: fail
  - Script: `set -e
cargo build -p myosu-play
cargo test -p myosu-games-poker
...
- failure_signature: verify|deterministic|script failed with exit code: <n> ## stderr blocking waiting for file lock on package cache blocking waiting for file lock on package cache blocking waiting for file lock on package cache finished `dev` profile [unoptimized + debuginfo] tar
