...
/// Report describing a successful subnet tempo override.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct TempoSetReport {
    pub subnet: NetUid,
    pub tempo: u16,
    pub extrinsic_hash: Option<String>,
...
/// Report describing a successful subnet weights-rate-limit override.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct WeightsRateLimitReport {
    pub subnet: NetUid,
...
/// Report describing a successful commit-reveal toggle.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct CommitRevealToggleReport {
    pub subnet: NetUid,
    pub enabled: bool,
    pub extrinsic_hash: Option<String>,
    pub already_set: bool,
...
/// Report describing a successful admin freeze window override.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct AdminFreezeWindowReport {
    pub window: u16,
    pub extrinsic_hash: Option<String>,
    pub already_set: bool,
}

/// Report describing a successful global network registration rate-limit override.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct NetworkRateLimitReport {
    pub network_rate_limit: u64,
    pub extrinsic_hash: Option<String>,
    pub already_set: bool,
}

/// Report describing a successful global subnet-limit override.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct SubnetLimitReport {
...
impl ChainClient {
    ...
    /// Ensures a subnet tempo override is applied through the local sudo path.
    pub async fn ensure_subnet_tempo_via_sudo(
        &self,
        signer_uri: &str,
        netuid: NetUid,
        tempo: u16,
        timeout: Duration,
    ) -> Result<TempoSetReport, ChainClientError> {
        if self.get_subnet_tempo(netuid).await? == tempo {
            return Ok(TempoSetReport {
                subnet: netuid,
                tempo,
                extrinsic_hash: None,
                already_set: true,
        ...
        let admin_call =
            runtime::RuntimeCall::AdminUtils(
                pallet_admin_utils::Call::<runtime::Runtime>::sudo_set_tempo { netuid, tempo },
            );
        let call = runtime::RuntimeCall::Sudo(pallet_sudo::Call::<runtime::Runtime>::sudo {
            call: Box::new(admin_call),
        });
        let watched = self
            .submit_signed_call_and_watch_inclusion(signer_uri, call, timeout)
            .await?;
        let Some(block) = self.chain_get_block(watched.inclusion_block).await? else {
            return Err(ChainClientError::ExtrinsicWatchFailed {
                operation: format!("sudo_set_tempo for subnet {netuid}"),
                detail: format!("missing inclusion block {}", watched.inclusion_block),
            });
        };
        let Some(extrinsic_index) = locate_block_extrinsic_index(
            ...
            return Err(ChainClientError::ExtrinsicWatchFailed {
                operation: format!("sudo_set_tempo for subnet {netuid}"),
                detail: format!(
                    "included at block {} but could not locate submitted extrinsic",
                    watched.inclusion_block
                ),
            });
        };
        let events = self.system_events_at(watched.inclusion_block).await?;
        for record in events {
            ...
            if let runtime::RuntimeEvent::System(frame_system::Event::ExtrinsicFailed {
                dispatch_error,
                ..
            }) = record.event
            {
                return Err(ChainClientError::ExtrinsicWatchFailed {
                    operation: format!("sudo_set_tempo for subnet {netuid}"),
                    detail: format!(
                        "dispatch_error={dispatch_error:?} at block {}",
                        watched.inclusion_block
                    ),
                });
            }
        }
        self.poll_for_subnet_tempo(netuid, tempo, timeout).await?;
    ...
    /// Ensures a subnet weights-set rate limit override is applied through the local sudo path.
    pub async fn ensure_weights_set_rate_limit_via_sudo(
        &self,
        signer_uri: &str,
        netuid: NetUid,
        weights_set_rate_limit: u64,
        timeout: Duration,
    ) -> Result<WeightsRateLimitReport, ChainClientError> {
        if self.get_weights_set_rate_limit(netuid).await? == weights_set_rate_limit {
            return Ok(WeightsRateLimitReport {
                subnet: netuid,
        ...
        let admin_call = runtime::RuntimeCall::AdminUtils(pallet_admin_utils::Call::<
            runtime::Runtime,
        ...
        let extrinsic_hash = self.submit_signed_call(signer_uri, call).await?;
        self.poll_for_weights_set_rate_limit(netuid, weights_set_rate_limit, timeout)
            .await?;

        Ok(WeightsRateLimitReport {
            subnet: netuid,
            weights_set_rate_limit,
            extrinsic_hash: Some(extrinsic_hash),
            already_set: false,
    ...
    /// Ensures commit-reveal is enabled or disabled through the owner admin path.
    pub async fn ensure_commit_reveal_weights_enabled(
        &self,
        signer_uri: &str,
        netuid: NetUid,
        enabled: bool,
        timeout: Duration,
    ) -> Result<CommitRevealToggleReport, ChainClientError> {
        if self.get_commit_reveal_weights_enabled(netuid).await? == enabled {
            return Ok(CommitRevealToggleReport {
                subnet: netuid,
                enabled,
                extrinsic_hash: None,
                already_set: true,
        ...
        let call = runtime::RuntimeCall::AdminUtils(
            pallet_admin_utils::Call::<runtime::Runtime>::sudo_set_commit_reveal_weights_enabled {
                netuid,
                enabled,
            },
        );
        let extrinsic_hash = self.submit_signed_call(signer_uri, call).await?;
        self.poll_for_commit_reveal_enabled(netuid, enabled, timeout)
            .await?;

        Ok(CommitRevealToggleReport {
            subnet: netuid,
            enabled,
            extrinsic_hash: Some(extrinsic_hash),
            already_set: false,
    ...
    /// Ensures the global admin freeze window is set through the local sudo path.
    pub async fn ensure_admin_freeze_window_via_sudo(
        &self,
        signer_uri: &str,
        window: u16,
        timeout: Duration,
    ) -> Result<AdminFreezeWindowReport, ChainClientError> {
        if self.get_admin_freeze_window().await? == window {
            return Ok(AdminFreezeWindowReport {
                window,
                extrinsic_hash: None,
                already_set: true,
            });
        ...
        let extrinsic_hash = self.submit_signed_call(signer_uri, call).await?;
        self.poll_for_admin_freeze_window(window, timeout).await?;

        Ok(AdminFreezeWindowReport {
            window,
            extrinsic_hash: Some(extrinsic_hash),
            already_set: false,
        })
    ...
    /// Ensures the global network registration rate limit is set through the local sudo path.
    pub async fn ensure_network_rate_limit_via_sudo(
        &self,
        signer_uri: &str,
        network_rate_limit: u64,
        timeout: Duration,
    ) -> Result<NetworkRateLimitReport, ChainClientError> {
        if self.get_network_rate_limit().await? == network_rate_limit {
            return Ok(NetworkRateLimitReport {
                network_rate_limit,
                extrinsic_hash: None,
                already_set: true,
            });
        ...
        let extrinsic_hash = self.submit_signed_call(signer_uri, call).await?;
        self.poll_for_network_rate_limit(network_rate_limit, timeout)
            .await?;

        Ok(NetworkRateLimitReport {
            network_rate_limit,
            extrinsic_hash: Some(extrinsic_hash),
            already_set: false,
        })
    ...
    /// Ensures the global subnet limit is set through the local sudo path.
    pub async fn ensure_subnet_limit_via_sudo(
        &self,
        signer_uri: &str,
        subnet_limit: u16,
        timeout: Duration,
    ) -> Result<SubnetLimitReport, ChainClientError> {
        if self.get_subnet_limit().await? == subnet_limit {
            return Ok(SubnetLimitReport {
                subnet_limit,
                extrinsic_hash: None,
                already_set: true,
            });
        ...
        >::sudo_set_subnet_limit {
            max_subnets: subnet_limit,
        });
        let call = runtime::RuntimeCall::Sudo(pallet_sudo::Call::<runtime::Runtime>::sudo {
            call: Box::new(admin_call),
        });
        let extrinsic_hash = self.submit_signed_call(signer_uri, call).await?;
        self.poll_for_subnet_limit(subnet_limit, timeout).await?;

        Ok(SubnetLimitReport {
            subnet_limit,
            extrinsic_hash: Some(extrinsic_hash),
            already_set: false,
        })
    ...
    async fn poll_for_subnet_tempo(
        &self,
        netuid: NetUid,
        tempo: u16,
        timeout: Duration,
    ...
    async fn poll_for_commit_reveal_enabled(
        &self,
        netuid: NetUid,
        enabled: bool,
        timeout: Duration,
        ...
        Err(ChainClientError::Timeout {
            operation: format!("sudo_set_commit_reveal_weights_enabled for subnet {netuid}"),
            timeout_secs: timeout.as_secs(),
        })
    }

    async fn poll_for_weights_set_rate_limit(
        &self,
        netuid: NetUid,
        weights_set_rate_limit: u64,
        timeout: Duration,
        ...
        Err(ChainClientError::Timeout {
            operation: format!("sudo_set_weights_set_rate_limit for subnet {netuid}"),
            timeout_secs: timeout.as_secs(),
        })
    }

    async fn poll_for_admin_freeze_window(
        &self,
        window: u16,
        timeout: Duration,
    ) -> Result<(), ChainClientError> {
        ...
        Err(ChainClientError::Timeout {
            operation: format!("sudo_set_admin_freeze_window to {window}"),
            timeout_secs: timeout.as_secs(),
        })
    }

    async fn poll_for_network_rate_limit(
        &self,
        network_rate_limit: u64,
        timeout: Duration,
    ) -> Result<(), ChainClientError> {
        ...
        Err(ChainClientError::Timeout {
            operation: format!("sudo_set_network_rate_limit to {network_rate_limit}"),
            timeout_secs: timeout.as_secs(),
        })
    }

    async fn poll_for_subnet_limit(
        &self,
        subnet_limit: u16,
        timeout: Duration,
    ) -> Result<(), ChainClientError> {
        ...
        Err(ChainClientError::Timeout {
            operation: format!("sudo_set_subnet_limit to {subnet_limit}"),
            timeout_secs: timeout.as_secs(),
        })
    }
