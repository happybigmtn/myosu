...
#[freeze_struct("b08427e49e06d17c")]
#[derive(Decode, Encode, PartialEq, Eq, Clone, Debug, TypeInfo)]
...
impl<T: Config> Pallet<T> {
    ...
    pub fn get_subnets_info_v2() -> Vec<SubnetInfov2<T::AccountId>> {
        ...
        let mut subnets_info = Vec::<SubnetInfov2<T::AccountId>>::new();
        for netuid_ in 0..=max_netuid {
            if subnet_netuids.contains(&netuid_.into()) {
                let Some(subnet_info) = Self::get_subnet_info_v2(netuid_.into()) else {
                    continue;
                };
                subnets_info.push(subnet_info);
            }
        }
