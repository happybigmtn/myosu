...
mod tests {
    ...
    fn resolve_key_uri_rejects_missing_key_source_after_cli_validation() {
        let mut cli = Cli::parse_from(["myosu-miner", "--subnet", "7", "--key", "//Alice"]);
        cli.key = None;
    ...
    fn cli_parses_research_portfolio_game_selection() {
        ...
        assert_eq!(cli.game, GameSelection::RiichiMahjong);
        assert_eq!(cli.game.portfolio_game(), Some(ResearchGame::RiichiMahjong));
    }
