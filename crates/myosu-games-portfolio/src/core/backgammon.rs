...
/// A single hand-verified backgammon benchmark scenario for the
/// `F-013` / `genesis/plans/009-cribbage-deepening.md` rule-aware
/// scenario pack.
///
/// Each scenario's expected recommendation is derived by hand from the
/// `state-aware race-contact heuristic` math in
/// `crate::engines::backgammon::answer`. The per-scenario docstring
/// records the bear_off / accept_double / advance_piece triple so a
/// reviewer can verify the dominant arm by hand.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct BackgammonScenario {
...
/// Return the canonical 22-scenario Backgammon benchmark pack.
///
/// Coverage requirements (mirrors `genesis/plans/009-cribbage-deepening.md`
/// R1 layout, scoped to the Backgammon engine surface):
/// - bear_off dominance × 7 (pristine-bearoff, clean-rno-double, thick,
///   medium-race, racing-home-empty, bearoff-cube-owned, clean-zero-blot)
/// - accept_double dominance × 7 (live-take, secure-take, deep-take,
///   clean-take-blotless, take-with-prime, cube-efficiency-rich,
///   small-bearoff-take)
/// - advance_piece dominance × 7 (contact-pristine, prime-and-bar,
///   contact-bar-entry, blotty-prime-midgame, crashed-contact-bar,
///   blot-rescue, contact-prime-deep)
/// - mixed/edge × 1 (cube-decision-with-prime — the live take where
///   `cube_owned_by_actor` narrows the gap between `bear_off` and
///   `accept_double`)
///
/// Total: 22 labeled scenarios, exceeds the 20-scenario floor. Each
/// scenario is hand-verified against the
/// `state-aware race-contact heuristic` math in
/// `crate::engines::backgammon::answer` so the dossier's expected
/// recommendations are stable across runs.
pub fn backgammon_scenario_pack() -> &'static [BackgammonScenario] {
    BACKGAMMON_SCENARIO_PACK
}

const BACKGAMMON_SCENARIO_PACK: &[BackgammonScenario] = &[
    // ---- bear_off bucket (×7) ----
    BackgammonScenario {
        scenario_id: "bo-pristine-bearoff",
        decision: "Pristine bearoff race with strong home-board + prime — bear_off dominates (bo=4.95, ad=2.20, ap=1.79)",
        race_lead_pips: 30,
        borne_off: 12,
        anchors: 0,
        cube_efficiency: 2,
    ...
    BackgammonScenario {
        scenario_id: "bo-clean-rno-double",
        decision: "Centered cube, no double faced, strong bearoff — bear_off dominates (bo=4.36, ad=1.92, ap=1.71)",
        race_lead_pips: 18,
        borne_off: 8,
        anchors: 0,
        cube_efficiency: 2,
    ...
    BackgammonScenario {
        scenario_id: "bo-bearoff-thick",
        decision: "Thick bearoff — 13 borne off, 4 home points, 2-prime — bear_off dominates (bo=5.10, ad=2.82, ap=1.83)",
        race_lead_pips: 25,
        borne_off: 13,
        anchors: 0,
        cube_efficiency: 2,
    ...
    BackgammonScenario {
        scenario_id: "bo-bearoff-medium-race",
        decision: "Mid bearoff with 5 off-moves — bear_off dominates (bo=4.29, ad=2.60, ap=1.89)",
        race_lead_pips: 12,
        borne_off: 10,
        anchors: 0,
        cube_efficiency: 2,
    ...
    BackgammonScenario {
        scenario_id: "bo-racing-home-empty",
        decision: "Empty home board, almost-bearoff race — bear_off dominates (bo=6.49, ad=2.92, ap=1.87)",
        race_lead_pips: 40,
        borne_off: 15,
        anchors: 0,
        cube_efficiency: 2,
    ...
    BackgammonScenario {
        scenario_id: "bo-bearoff-cube-owned",
        decision: "Actor owns the cube and has a strong bearoff — bear_off dominates (bo=4.81, ad=1.90, ap=2.17)",
        race_lead_pips: 15,
        borne_off: 12,
        anchors: 0,
        cube_efficiency: 2,
    ...
    BackgammonScenario {
        scenario_id: "bo-bearoff-clean-zero-blot",
        decision: "Zero-blot clean bearoff — bear_off dominates (bo=4.82, ad=2.30, ap=1.65)",
        race_lead_pips: 22,
        borne_off: 11,
        anchors: 0,
        cube_efficiency: 2,
    ...
    // ---- accept_double bucket (×7) ----
    BackgammonScenario {
        scenario_id: "ad-live-take",
        decision: "Live take of a double with strong immediate bearoff — accept_double dominates (bo=3.87, ad=4.85, ap=1.65)",
        race_lead_pips: 10,
        borne_off: 11,
        anchors: 0,
        cube_efficiency: 2,
    ...
    BackgammonScenario {
        scenario_id: "ad-secure-take",
        decision: "Secure take with home-board coverage — accept_double dominates (bo=3.06, ad=4.29, ap=1.89)",
        race_lead_pips: 8,
        borne_off: 10,
        anchors: 0,
        cube_efficiency: 2,
    ...
    BackgammonScenario {
        scenario_id: "ad-deep-take",
        decision: "Deep take of a double with mid-bearoff continuation — accept_double dominates (bo=3.26, ad=4.61, ap=1.89)",
        race_lead_pips: 8,
        borne_off: 8,
        anchors: 0,
        cube_efficiency: 2,
    ...
    BackgammonScenario {
        scenario_id: "ad-clean-take-blotless",
        decision: "Blotless take with 5 off-moves and mid race — accept_double dominates (bo=3.42, ad=4.45, ap=1.89)",
        race_lead_pips: 15,
        borne_off: 10,
        anchors: 0,
        cube_efficiency: 2,
    ...
    BackgammonScenario {
        scenario_id: "ad-take-with-prime",
        decision: "Take with prime + 4 off-moves + 2 home points — accept_double dominates (bo=3.03, ad=4.25, ap=1.99)",
        race_lead_pips: 12,
        borne_off: 9,
        anchors: 0,
        cube_efficiency: 2,
    ...
    BackgammonScenario {
        scenario_id: "ad-cube-efficiency-rich",
        decision: "Take with double-cube efficiency (efficiency=3) — accept_double dominates (bo=2.97, ad=4.36, ap=1.59)",
        race_lead_pips: 9,
        borne_off: 10,
        anchors: 0,
        cube_efficiency: 3,
    ...
    BackgammonScenario {
        scenario_id: "ad-small-bearoff-take",
        decision: "Small-bearoff take with 1 blot, 4 off-moves — accept_double dominates (bo=2.67, ad=4.01, ap=1.19)",
        race_lead_pips: 4,
        borne_off: 8,
        anchors: 0,
        cube_efficiency: 2,
    ...
    // ---- advance_piece bucket (×7) ----
    BackgammonScenario {
        scenario_id: "ap-contact-pristine",
        decision: "Contact position with bar + 2 anchors — advance_piece dominates (bo=0.76, ad=-0.55, ap=3.63)",
        race_lead_pips: -6,
        borne_off: 4,
        anchors: 2,
        cube_efficiency: 0,
    ...
    BackgammonScenario {
        scenario_id: "ap-prime-and-bar",
        decision: "Deep contact with 3-prime + bar entry pending — advance_piece dominates (bo=0.68, ad=-0.49, ap=4.53)",
        race_lead_pips: -15,
        borne_off: 2,
        anchors: 2,
        cube_efficiency: 0,
    ...
    BackgammonScenario {
        scenario_id: "ap-contact-bar-entry",
        decision: "Contact bar-entry with 2 blots to clear — advance_piece dominates (bo=0.57, ad=-0.99, ap=2.92)",
        race_lead_pips: -5,
        borne_off: 1,
        anchors: 1,
        cube_efficiency: 0,
    ...
    BackgammonScenario {
        scenario_id: "ap-blotty-prime-midgame",
        decision: "Midgame contact with 3 blots and a 2-prime — advance_piece dominates (bo=0.85, ad=-0.95, ap=2.75)",
        race_lead_pips: -12,
        borne_off: 2,
        anchors: 2,
        cube_efficiency: 0,
    ...
    BackgammonScenario {
        scenario_id: "ap-crashed-contact-bar",
        decision: "Crashed contact with 2-on-bar and a facing double — advance_piece dominates (bo=-1.37, ad=-0.82, ap=1.68)",
        race_lead_pips: -20,
        borne_off: 0,
        anchors: 3,
        cube_efficiency: 0,
    ...
    BackgammonScenario {
        scenario_id: "ap-blot-rescue",
        decision: "Mid-race contact with 2 blots to rescue — advance_piece dominates (bo=0.65, ad=-1.20, ap=2.49)",
        race_lead_pips: -8,
        borne_off: 2,
        anchors: 2,
        cube_efficiency: 0,
    ...
    BackgammonScenario {
        scenario_id: "ap-contact-prime-deep",
        decision: "Deep contact with 3-prime + 1 blot to clear — advance_piece dominates (bo=0.55, ad=-0.75, ap=3.51)",
        race_lead_pips: -18,
        borne_off: 0,
        anchors: 2,
        cube_efficiency: 0,
    ...
    // ---- mixed/edge (×1) ----
    BackgammonScenario {
        scenario_id: "mixed-cube-decision-with-prime",
        decision: "Cube decision with cube_owned_by_actor narrowing the gap between bear_off and accept_double (bo=2.89, ad=3.56, ap=1.69) — accept_double still dominates by 0.67 but the gap is the narrowest in the pack",
        race_lead_pips: 6,
        borne_off: 10,
        anchors: 0,
        cube_efficiency: 1,
