...
2. **Spec corpus was actively misleading at audit time.** `specs/031626-07-tui-implementation.md` and `specs/031626-10-agent-experience.md` were empty, `specs/031626-11` and `specs/031626-12` were byte-identical duplicates, and 13 non-numbered mirror copies sat beside the numbered canonicals. Later `002` work fixed the empty/duplicate file problem, but freshness drift still required a follow-up sync.
...
6. **The robopoker dependency is an external fork.** `happybigmtn/robopoker`
   provides `rbp-core`, `rbp-mccfr`, `rbp-nlhe`, `rbp-cards`, `rbp-gameplay`.
   Pinned via git rev. INV-006 governs fork coherence. The repo now carries
   `docs/robopoker-fork-changelog.md` documenting divergence from the
   `v1.0.0` baseline at the pinned workspace rev.
...
|---------|----------|--------|
| Canonical spec freshness drift | The namespace cleanup from `002` removed the empty and duplicate files, but `specs/031626-00-master-index.md` and downstream Genesis summaries still carried stale pre-implementation language until this sync | Doctrine readers can misread current implementation state |
| Hosted CI closure proven | The workflow is now published and hosted run `23741634642` finished fully green on the current repo surface | `010` is closed with real GitHub Actions timing evidence under the `<15 min` target |
...
| Agent experience | Agent transports described in OS.md and captured in spec form, but no implementation exists yet | Spec exists, code does not |

## Tech Debt Inventory

| # | Area | Debt | Evidence | Impact | Owning Plan |
|---|------|------|----------|--------|-------------|
| TD-01 | Spec corpus | Canonical namespace is clean, but master index and downstream summaries still need freshness sync against current implementation state | `specs/031626-00-master-index.md`, `genesis/GENESIS-REPORT.md`, `genesis/ASSESSMENT.md` | Planning integrity | 002 |
| TD-02 | Doctrine freshness gate | CI enforces spec presence and plan structure, but not whether canonicals still describe current repo truth | `.github/scripts/check_doctrine_integrity.sh` proves structure, not freshness | Future stale doctrine can survive longer than it should | 002 |
...
| TD-08 | Robopoker fork | Fork coherence now uses `docs/robopoker-fork-changelog.md`, but that file must stay synced with any future pin change | INV-006 | Drift between pin and documented fork delta | 006 |
...
| SR-05 | Hosted CI blind spot materially reduced | Hosted runner evidence now exists on the current repo surface; run `23741634642` passed every workflow job, including `Chain Core` and `Chain Clippy`, under the `<15 min` target | Low | 010 |
| SR-06 | Key management remains partial | `myosu-keys` now covers encrypted at-rest storage, encrypted keyfile import/export, mnemonic/raw-seed env-var import, active-account password rotation, printed miner/validator bootstrap commands, a generated operator wrapper/spec bundle with bundle-local verification, machine-readable metadata, materialized named-network JSONs, and a hosted CI lane definition, plus minimal account creation/list/switch CLI flows and miner/validator keystore consumption, but it still lacks a broader player wallet lifecycle | Medium | 021 |
...
- Canonical spec freshness is still manual; current CI verifies presence and
  indexing, not whether spec prose still matches the current implementation
...
Myosu is now a locally and hosted-proven two-game stage-0 chain loop with the
remaining weaknesses shifted away from CI closure and toward the still-messy
canonical spec corpus plus broader product prioritization.
...
|------|--------|---------|-------|
| 031626-00-master-index | Active | Medium | Canonical and non-empty, but still needs current-state freshness work after carrying greenfield language too long |
...
| 031626-07-tui-implementation | Active | Medium | Restored on 2026-03-29; no longer empty |
...
| 031626-10-agent-experience | Active | Medium | Restored on 2026-03-29; no longer empty |
| 031626-11-agent-coordination | Active | Low | Narrowed future topic with supersession note; no longer duplicates 031626-12 |
| 031626-12-nlhe-incentive | Active | Medium | Canonical incentive spec; no longer duplicated by 031626-11 |
...
| Non-numbered mirrors | Archived | Low | Moved to `specsarchive/`; not part of the active control plane anymore |
