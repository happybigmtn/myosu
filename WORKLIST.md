...
- `NET-FINALITY-001` Investigate why GRANDPA finality stalls after one authority is stopped on the 3-authority `devnet`. On 2026-04-05, draft repros based on a 3-node harness showed best blocks continuing to import while finalized height froze (`#1` and `#2` in two separate runs), with node logs ending in `Backing off claiming new slot for block authorship: finality is lagging`.
