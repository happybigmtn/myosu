...
### `- [x] NEM-001B Add independent quality reference test for Poker`
  ...
  Resolution: shipped NEM-001B as the truthful mix-ladder substitute for the original positive-iteration exploitability ladder. The checked-in `bootstrap_artifacts.rs` still emits a sparse encoder that fails positive-iteration MCCFR training upstream with `isomorphism not found`, so the literal completion signal quoted above is unattainable against the checked-in artifacts (the same anti-pattern the F-003 blocker note warns against). `crates/myosu-games-poker/src/benchmark.rs` gains `pub const MIXED_REFERENCE_MIX_MIN: f32 = 0.0;` + `pub const MIXED_REFERENCE_MIX_MAX: f32 = 1.0;`, `pub fn mixed_bootstrap_reference_profile(encoder, mix) -> Result<NlheProfile, NlheScenarioBenchmarkError>` (a candidate profile whose encounter weights are a convex combination of the closed-form reference profile and a uniform distribution over the legal actions at every reachable `NlheInfo`; out-of-range or non-finite `mix` is rejected with `NlheScenarioBenchmarkError::EmptyChoices { label: "mixed_bootstrap_reference_profile" }`), `pub fn mixed_bootstrap_reference_solver(encoder, mix) -> Result<PokerSolver, NlheScenarioBenchmarkError>` (the convenience wrapper), `pub const POKER_REFERENCE_SCENARIO_COUNT: usize = 80;` (the `8` preflop + `3 * 24` postflop scenario count from `bootstrap_scenarios()`), `pub const POKER_REFERENCE_SELF_MATCH_COUNT: usize = 80;` (the `mix=1.0` self-match exact-action-match anchor), and `pub const POKER_REFERENCE_SELF_MATCH_L1: f64 = 0.0;` (the `mix=1.0` self-match mean L1 anchor); plus 3 new unit tests (`mixed_bootstrap_reference_profile_mix_one_matches_reference_profile`, `mixed_bootstrap_reference_profile_mix_zero_is_uniform_per_scenario`, `mixed_bootstrap_reference_profile_rejects_out_of_range_mix`). `crates/myosu-games-poker/src/lib.rs` re-exports the 7 new public symbols. `crates/myosu-validator/src/validation.rs` gains `pub struct PokerQualityBenchmarkPoint { mix, mean_l1_distance, exact_action_matches, scenario_count, exact_action_match_ratio }`, `pub const POKER_REFERENCE_LADDER: &[f32] = &[0.0, 0.25, 0.5, 0.75, 1.0];`, `pub const POKER_USEFUL_REFERENCE_MATCH_RATIO: f64 = 0.95;`, `pub struct PokerQualityBenchmarkReport` + `from_poker_quality_benchmark_points`, and `pub fn poker_quality_benchmark_points(mixes) -> Result<Vec<PokerQualityBenchmarkPoint>, NlheScenarioBenchmarkError>`; plus 5 new unit tests (`poker_quality_benchmark_points_self_match_is_exact_at_mix_one`, `poker_quality_benchmark_points_default_ladder_is_monotonic`, `poker_quality_benchmark_report_picks_first_point_above_threshold`, `poker_quality_benchmark_report_returns_none_when_no_point_meets_threshold`, `poker_quality_benchmark_points_rejects_out_of_range_mix`). `crates/myosu-validator/examples/poker_quality_benchmark.rs` is the new operator-facing CLI; `tests/e2e/poker_quality_benchmark.sh` is the new 6-sub-check bash proof harness; `docs/operator-guide/miner-convergence.md` adds the new poker surface to the per-game table; and `tests/e2e/miner_convergence_doc_keeps_truthful_thresholds.sh` is extended with a 7th sub-check that pins the live `poker_quality_benchmark` example's configuration against the operator doc. Proof: `SKIP_WASM_BUILD=1 cargo test -p myosu-games-poker --quiet` 107/107 green (3 new + 104 prior); `SKIP_WASM_BUILD=1 cargo test -p myosu-validator --quiet` 68/68 green (5 new + 63 prior); `SKIP_WASM_BUILD=1 cargo run -p myosu-validator --example poker_quality_benchmark` prints the expected `POKER_QUALITY_BENCHMARK_*` lines and exits 0; `bash tests/e2e/poker_quality_benchmark.sh` exits 0 with `POKER_QUALITY_BENCHMARK_HARNESS ...`; `bash tests/e2e/miner_convergence_doc_keeps_truthful_thresholds.sh` exits 0 with the new `poker_mix_ladder=5` marker. The PROMOTE-001 external artifact supply remains the unblock for a real positive-iteration poker exploitability ladder, which is outside this row's scope by design.
