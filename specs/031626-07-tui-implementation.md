...
- `crates/myosu-tui/` already exists as the live shared shell for `myosu-play`
- `myosu-play` already uses the shared TUI plus game-specific renderers for the
  current interactive and pipe surfaces

This spec adds:
- the truthful design contract for the shared TUI crate that now exists
- the remaining parity and evolution rules for the shell, renderer trait, and
  pipe surface
- the ownership map for the actual `myosu-tui` module layout in-repo
...
- design.md defines the broader visual contract
- `crates/myosu-tui/src/lib.rs` is the live shared TUI crate root
- `crates/myosu-tui/src/renderer.rs` defines the current shared renderer
  contract
- `crates/myosu-tui/src/shell.rs`, `src/events.rs`, `src/input.rs`,
  `src/screens.rs`, `src/theme.rs`, and `src/pipe.rs` form the current shell,
  event, and pipe surfaces
- `crates/myosu-play/` already consumes `myosu-tui` for the current
  interactive and agent-facing gameplay paths
...
|-----------|--------|----------|
| TUI crate | Implemented | crates/myosu-tui/src/lib.rs |
| GameRenderer trait | Implemented | crates/myosu-tui/src/renderer.rs |
| Shell (5-panel layout) | Implemented | crates/myosu-tui/src/shell.rs |
| Event loop | Implemented | crates/myosu-tui/src/events.rs |
| Input handler | Implemented | crates/myosu-tui/src/input.rs |
| Screen state machine | Implemented | crates/myosu-tui/src/screens.rs |
| Pipe mode | Implemented | crates/myosu-tui/src/pipe.rs |
| Theme / semantics | Implemented | crates/myosu-tui/src/theme.rs |
| Shared schema surface | Implemented | crates/myosu-tui/src/schema.rs |
| NLHE renderer (reference) | Implemented | crates/myosu-games-poker/src/renderer.rs |
